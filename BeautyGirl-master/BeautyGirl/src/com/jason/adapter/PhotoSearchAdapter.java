 ...
 */
public class PhotoSearchAdapter extends BaseAdapter {

    private Context mContext;
    private List<PhotoSearchObject> list;
    private ImageLoader imageLoader;
    private ViewHolder viewHolder;
    private int ScreenWidth;

    public PhotoSearchAdapter(Context mContext, List<PhotoSearchObject> list) {
    ...
    public void updateAdapter(List<PhotoSearchObject> list) {
        this.list = list;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        int h = (ScreenWidth * (list.get(position).getHeight())) / (2 * list.get(position).getWidth());
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(w, h);
        viewHolder.thumbImage.setLayoutParams(params);
        //设置图片描述
        viewHolder.title_tag.setText(list.get(position).getFromPageTitleEnc());
        //加载图片
        imageLoader.displayImage(list.get(position).getObjURL(), viewHolder.thumbImage,
